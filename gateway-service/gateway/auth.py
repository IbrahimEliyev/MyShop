...
async def handle_login(request: Request):
    ...
    async with httpx.AsyncClient() as client:
        user_res = await client.post(
            f"{SERVICE_URLS['user']}/api/user/login/",
            json=body
        )

    if user_res.status_code != 200:
        return JSONResponse(user_res.json(), status_code=user_res.status_code)
    ...
    if not user_uuid:
        return JSONResponse(
            {"detail": "User UUID missing from user service"},
            status_code=500
        )

    # 2️⃣ Fetch shop UUID (USING YOUR FUNCTION LOGIC)
    shop_uuid = None
    try:
        async with httpx.AsyncClient() as client:
        ...
        logger.info(
            f"Shop lookup response: status={shop_res.status_code}, body={shop_res.text}"
        )
        ...
        logger.error(f"Shop lookup failed: {e}")
    ...
    # ✅ ONLY include shop_uuid if it exists
    if shop_uuid:
        payload["shop_uuid"] = shop_uuid
